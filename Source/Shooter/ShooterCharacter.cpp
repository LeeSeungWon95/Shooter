	...
	HighlightedSlot(-1)
{
	...
	CameraBoom->bUsePawnControlRotation = true; // Rotate cameraboom based on the controller
...
void AShooterCharacter::BeginPlay()
{
	...
	// Create FInterpLocation sturcts for each interp location. Add to array
...
void AShooterCharacter::AutoFireReset()
{
	...
	if (WeaponHasAmmo())
	{
		if (bFireButtonPressed && EquippedWeapon->GetAutomaic())
		{
			FireWeapon();
		}
	}
	...
	if (bShouldTraceForItems)
	{
		...
		if (ItemTraceResult.bBlockingHit)
		{
			...
			if (TraceHitWeapon)
			{
				...
				{
					// Not currently highlighting a slot; highlighted one
...
void AShooterCharacter::FinishReloading()
{
	...
	if (AmmoMap.Contains(AmmoType))
	{
		...
		const int32 MagEmptySpace = EquippedWeapon->GetMagzineCapacity() - EquippedWeapon->GetAmmo();
...
void AShooterCharacter::InterpCapsuleHalfHeight(float DeltaTime)
{
	...
	const float InterpCapsuleHalfHeight{ FMath::FInterpTo(GetCapsuleComponent()->GetScaledCapsuleHalfHeight(),
		TargetCapsuleHalfHeight, DeltaTime, 20.f) };

	// Negative value if crouching; Positive value if standing
	const float DeltaCapsuleHalfHeight{ InterpCapsuleHalfHeight - GetCapsuleComponent()->GetScaledCapsuleHalfHeight() };
...
{
	bool bCanExchangeItems = (CurrentItemIndex != NewItemIndex) &&
